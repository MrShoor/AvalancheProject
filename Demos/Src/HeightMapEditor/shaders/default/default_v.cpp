...
HS_ConstOut EdgeDistanceConstantFunction(InputPatch<VS_Data, 4> Patch, uint patchId : SV_PrimitiveID) {    
    ...
    k[0] = length(vCrd[0].xyz - vCrd[1].xyz)/(vCrd[0].z + vCrd[1].z);
    k[1] = length(vCrd[1].xyz - vCrd[3].xyz)/(vCrd[1].z + vCrd[3].z);
    k[2] = length(vCrd[3].xyz - vCrd[2].xyz)/(vCrd[3].z + vCrd[2].z);
    k[3] = length(vCrd[2].xyz - vCrd[0].xyz)/(vCrd[2].z + vCrd[0].z);
...
[domain("quad")]
VS_Output DS(HS_ConstOut ConstIn, float2 uvCoord: SV_DomainLocation, OutputPatch<VS_Data, 4> patch) {
    VS_Data In;
    ...
    GetMapCoordWithNormal(coord2D, Out.wCoord, Out.vNorm);
